...
class StoriesController < ApplicationController
  ...
  def index
    ...
    @stories = @stories.where(asser_id: current_user.id) if params[:author].present? && params[:author] == 'Only Me'
  ...
  def story_params
    params.require(:story).permit(:address, :place_name, :type_of_stuff, :category, :rating, :story_description, media: [], asser_id: @asser.id)
  end

  def generate_csv(stories)
    CSV.generate(headers: true) do |csv|
      csv << ['Place Name', 'Type of Stuff', 'Category', 'Rating', 'Story Description', 'Media']
